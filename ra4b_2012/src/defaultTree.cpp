...
//void defaultTree::Fill( EventInfo* info, EasyChain* tree, vector<Muon*> muons, vector<Electron*> electrons, vector<Jet*> jets, double& met){
  ...
  for (unsigned int iel=0; iel<electrons.size();++iel){
    pt1=electrons.at(iel)->Pt();
    pt2=met.pt();
    MTEle=sqrt( 2*pt1*pt2 * (1 - cos(DeltaPhi(electrons.at(iel)->P4(), met))));
    break;
  }
  ...
  for (unsigned int imu=0; imu<muons.size();++imu){
    pt1=muons.at(imu)->Pt();
    pt2=met.pt();
    MTMu=sqrt( 2*pt1*pt2 * (1 - cos(DeltaPhi(muons.at(imu)->P4(), met))));
    break;
  }
